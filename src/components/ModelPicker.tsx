...
import { Cpu, Check, ChevronDown, Loader2, Info } from "lucide-react";
import { useStore } from "@/lib/store";
import { cx } from "@/lib/utils";
import { loadModels, selectModel, modelLabel, shortModelId, tierRank, type AiModel, type AiSurface } from "@/lib/models";
...
export default function ModelPicker({ surface, value, onChange, className }: Props) {
  ...
  const pick = async (m: AiModel) => {
    ...
    try {
      ...
      toast("error", "Could not save model", err instanceof Error ? err.message : "Try again");
    } finally {
      setSaving(false);
    }
  };
