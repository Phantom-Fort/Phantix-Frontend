...
│  No company JWT required on these calls                         │
│                                                                 │
...
│                                                                 │
│  Store: org_user_jwt + dual_control_session                     │
└────────────────────────────┬────────────────────────────────────┘
                             ▼
┌─────────────────────────────────────────────────────────────────┐
...
### Phase 1 — Create dual-control candidates
...
| Field | Guidance |
|-------|----------|
| `email` | Prefer **work domain** (`@company.com`). Free-mail only if it matches registration contact / primary / secondary (domain-exempt). |
...
- Wizard step: “Add **Initiator**” then “Add **Authorizer**”.
- Enforce different emails in the form.
- After create, list users: `GET /org-users` (look for `otp_only: true`).
- Show badges once assigned: “Initiator” / “Authorizer” from dual-control GET.
- After `PUT /org-users/dual-control`, prompt initiator and authorizer to **re-login**
  with `purpose=dual_control` (old operate sessions are revoked).

**Optional later:** add viewers with `role: "viewer"` — they never get dual-control slots unless reassigned.

---

### Phase 2 — Assign initiator & authorizer
