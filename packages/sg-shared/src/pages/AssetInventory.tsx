...
export interface AssetInventoryProps {
  /** Page heading, named for the application showing it. */
  title?: string;
}
...
export default function AssetInventory({ title = "Assets" }: AssetInventoryProps) {
  ...
  const discoveryForAsset = (a: Asset): { job_type: string; config: Record<string, unknown>; target_asset_id?: number } | null => {
    switch (a.asset_type) {
      ...
      // IP addresses have no automatic surface to enumerate (policy: nothing for
      // an IP). A discovered port_service above can still be re-probed.
      case "mobile_apk":
  ...
  return (
    <div>
      ...
      <PageHeader
        title={title}
        description="Your complete in-scope asset inventory. Discovery is gated so dead hosts never enter the list."
        actions={
            ...
            <Card key={j.id}>
              ...
              {tools.length > 0 && (
                <div className="mt-2 text-[12px] text-slate-500">
                  Tools: {tools.join(", ")}
                  {rs.method && <span className="ml-2">· Request type: {rs.method}</span>}
                </div>
              )}
      ...
      <Modal open={showApiModal} onClose={() => setShowApiModal(false)} title="Import OpenAPI / Postman">
        <div className="space-y-3">
          ...
          <p className="text-[12px] text-slate-500">Imported entries become API assets with their connection details.</p>
          <button
